...
- **Configurable AI context window.** User-editable setting for how many weeks of history the AI sees (default 4, range 1-12). Useful if the user takes a break and wants the AI to see pre-break performance rather than interpret the gap as detraining. Stored as a user preference (new table `user_settings` or column on an existing table). *Idea only — no design.*
