...
- **View Recent — remaining mismatches.** The `v2.0.10` resolver (`resolveLibraryRow` + `EXERCISE_ALIASES`) handles most plan-to-seed name mappings, but some exercises still show "no history." Needs continued debugging of the generated resolver candidates vs `exerciseLibraryByName` entries for specific failing exercises — likely orphan user-custom rows intercepting (see HANDOFF.md → "Known v1 limitations → Open") or alias gaps. *Low priority* — the History browser works correctly as the primary way to browse past workouts; View Recent is a convenience surface on top of individual exercise cards.
