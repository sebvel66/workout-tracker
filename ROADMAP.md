...
- **Historical ad-hoc browsing.** Today's ad-hoc sessions render as extra tabs, but past ad-hocs are invisible in the UI (the current historical-day path filters by `day_index`, which is NULL for ad-hoc). Needs a date-based or activity-log view to surface past off-plan sessions. *Idea only.*
