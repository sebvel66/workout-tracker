...
Total ~16,720 lines split across 6 frontend files + 3 server files. Rough sizes (as of v3.6.13):

| File | ~Lines | What lives here |
|---|---|---|
| `js/resolver.js` | 82 | `normName`, `EXERCISE_ALIASES`, `resolveLibraryRow`. Pure string/lookup utility. |
| `js/data.js` | 4862 | Supabase client (`sb`), cross-cutting state, queries, persistence, session lifecycle, plan activation/import (`savePlanAsActive` / `activateExistingPlan` / `endActivePlan`), `fetchWeekSummary`, `fetchRecentWorkouts`, `fetchVolumeTrends`, photo CRUD, coach message persistence, coaching profile load/save, drop-set state model, cardio helpers, hydration cache, **superset block management** (`applySupersetMerge` / `applySupersetSeparate` / `applySupersetReorderMembers` / `addRoundToBlockMembers` / `_resolveFlatEi` / `_flatEiToPlanMember` / `_restateFromSupersetGroups` / `shouldFireRestForBlockMember` / `supersetGroupsFromPlanDay`), `modelForCoach` / `modelForPlan` / `modelForAnalyze` resolvers, weight-mode override fan-out (`setExerciseWeightMode`), history-edit helpers (`historyUpdateSetField` / `historyAddSet` / `historyDeleteSet` etc.), form notes (`loadFormNotes` / `loadFormNotesBatch` / `saveUserFormNote` / `saveAiFormNote` / `generateAiFormNote`), `expandSetRepeatsInPlan` for import-path canonicalization, `persistSet` with v3.6.4 read-before-insert defense. |
| `js/ui.js` | 8733 | Renderers, modals (History, Photos, Plans, Templates, Generate, Coaching Profile, Coach Chat panel, Resume-Session prompt, Superset picker), event listeners, toast, session timer, rest-timer pill (drag-to-move with `_restPillOffsetX/Y` persistence across pointerdowns), export, refine flow, profile-update diff cards, drop-set UI, no-plan empty-state (`renderEmptyState` / `renderEmptyStateRecent`), **superset rendering** (`groupRunsForRender` / `renderSupersetBlock` / `renderPlanDayExerciseCard` / `renderPlanDayExtraCard` / `renderAdHocExerciseCard` / `renderHistoryExerciseCard`), per-block SortableJS zones, History edit mode (with `invalidateHistoryWeekCache` for in-modal handlers vs `invalidateHistoryCache` for full-detail-drop paths — see MAJOR-BUGS.md 2026-05-10), **inline form notes** (`renderInlineFormNotes` / `hydrateFormNotesForDay` / `resolveCardExerciseId` + `formNotesCache` shared with the modal). |
| `js/auth.js` | 172 | OTP form state + handlers, `applySession` (sign-in → cache reset → hydrate). |
...
- **v3.4.1 – v3.4.12 (2026-05-04 → 2026-05-07):** patch line layered on top of v3.4.0 supersets. Bug fixes (recent-workouts row click, refine-in-no-plan, in-review block-aware swap, plan-id-scoped historical lookup, analyze-form input persistence, stable chat ordering across restart, per-plan rollup in History) plus a coordinated thread that makes the active plan's day tab editable pre-session without mutating `plan.data`. Pre-session +Add Set / +Drop Set lazy-init `todayState` + `exState` and add session-scoped extras (`isExtra: true`); the existing `toggleSet` cascade persists them when the session is started. Pre-session `⟷` chain-link merge / unpair drives `applySupersetMerge` / `applySupersetSeparate` (already gated on `todayPlanState.workoutId` so DB writes are skipped pre-session). `deleteSet` cascade-collects drop children of a non-drop parent (DB cascade via `parent_set_id ON DELETE CASCADE` on persisted children; in-memory splice + `parentSetIdx` shift for survivors). Two latent issues surfaced and fixed alongside: `addExtraSet` / `addDropSet` / `_addDropSetInner` now scan for the actual last populated index instead of using `length - 1`, and the +Drop Set button gate flipped from `length > 0` to `.some(s => s != null)`. v3.4.11 closed a coach-chat ordering race by client-stamping `created_at` on the user and assistant inserts; v3.4.12 added `plansBreakdown` to `fetchWeekSummary` so multi-plan weeks aggregate correctly.
- **v3.5.0 – v3.5.9 (2026-05-07 → 2026-05-09):** volume-by-muscle analytics milestone end-to-end, plus several quality-of-life additions in the same window. **v3.5.0** adds Phase 1a primary-muscle counting (`_setsByMuscleGroup` per workout, `volumeByMuscleGroup` per week summary, History line, server-side `formatVolumeByMuscleGroup`, `## VOLUME BY MUSCLE GROUP` section in `system-prompt-analyze.md`). **v3.5.1** ships Phase 1b: new migration `20260509000000_secondary_muscles.sql` adds `exercises.secondary_muscles text[]` with a CHECK constraint, backfills seed rows via heuristic UPDATEs grouped by `(muscle_group, movement_pattern)` plus name-pattern overrides, and switches the counting to Schoenfeld primary 1.0 + each secondary 0.5 across frontend + server. **v3.5.2** adds a user-configurable coach-history window (`coaching_profile.data.coach_context_weeks`, 1-12) consumed by `buildCoachContext` (frontend) and `handleSwap` (server, via POST payload). **v3.5.3** adds `mode='analyze_chat'` on `/api/generate-plan` riding the warm analyze cache for in-modal Q&A follow-ups; system-prompt-analyze.md gains a `## TWO REQUEST TYPES` section dispatching on the trailer (JSON vs free-form). **v3.5.4** ships the Volume Trends dashboard (hamburger entry → modal table with sparklines, 4w/8w/12w window selector, single Supabase query bucketed client-side via `fetchVolumeTrends`). **v3.5.5** adds a per-muscle volume chip block to the plan-gen review color-coded against the phase target band (`computePlanVolumeByMuscle` + `phaseTargetBand` helpers; same code path for initial generate AND post-refine review). **v3.5.6** injects a `SAVED TEMPLATES` block into coach-chat / analyze / analyze_chat user messages (compact: name + per-day exercise names, soft-cap 10 most recent); `fetchUserTemplates` + `formatUserTemplates` mirrored across `api/coach-chat.js` and `api/generate-plan.js` per the helper-mirror convention. **v3.5.7** adds Import Template (mirrors Import Plan, accepts three input shapes, prompts for name, calls existing `saveAsTemplate`). **v3.5.8** adds per-exercise form notes via migration `20260509010000_exercise_form_notes.sql` (`(user_id, exercise_id)` PK, owner-RLS, columns `user_note` / `ai_note` / `ai_generated_at`); the existing "view recent" exercise modal gains Recent / Form tabs; new helpers `loadFormNotes` / `saveUserFormNote` / `saveAiFormNote` / `generateAiFormNote`. **v3.5.9** adds "Use as plan" per template row in the Templates modal so multi-day templates can become the active plan via existing `savePlanAsActive` (deep-clones template data, strips `start_date`/`week`).
- **v3.6.0 – v3.6.2 (2026-05-09):** bottom tab navigation milestone. Three primary destinations promoted to a fixed bottom tab bar — **Workout** (existing tracker, unchanged), **Body** (per-muscle weekly volume + targets, two chip blocks: actuals from `fetchWeekSummary().volumeByMuscleGroup` + projection from `computePlanVolumeByMuscle(plan)`), **Log** (launchpad cards opening the existing History modal + Volume Trends modal — inlining the full content of those views deferred). Markup: existing workout content wrapped in `#workoutView`; new `#bodyView` + `#logView` containers; new `<nav class="bottom-tab-bar">`. CSS uses `body[data-view="X"]` selectors to swap view containers; floating elements (summary bar, coach FAB, rest pill) repositioned `+60px` above the tab bar; `body.unauthed` hides the bar so the auth-gate renders cleanly. Coach FAB stays as always-on companion; hamburger menu unchanged. **v3.6.1** hardens no-plan + sign-out paths (bumped workout-container/empty-state bottom padding to clear the tab bar; sign-out resets `data-view` and `activeView`; unauthed selector hides the new view containers). **v3.6.2** adds drag-to-move on the rest-timer pill via pointer events with a 5px threshold + button-region exclusion; position resets to default on every new rest period (`startRestTimer` clears the inline `transform`).

End of v3.6.2: ~16,010 total lines across `js/*` + `api/*` (vs ~4,500 at end of Session B, ~14,470 at end of v3.4.10). v3.5 added ~1,200 lines (volume helpers across `data.js`, Volume Trends + plan-volume-summary + analyze-chat in `ui.js`, `formatVolumeByMuscleGroup` + analyze_chat handler + templates helpers in `api/*`); v3.6 added ~330 lines (tab-nav state + Body view + Log view + drag handlers in `ui.js`, plus tab bar HTML/CSS in `index.html`).
