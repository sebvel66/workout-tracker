...
- **v3.4.1 – v3.4.10 (2026-05-04 → 2026-05-06):** patch line layered on top of v3.4.0 supersets. Bug fixes (recent-workouts row click, refine-in-no-plan, in-review block-aware swap, plan-id-scoped historical lookup, analyze-form input persistence) plus a coordinated thread that makes the active plan's day tab editable pre-session without mutating `plan.data`. Pre-session +Add Set / +Drop Set lazy-init `todayState` + `exState` and add session-scoped extras (`isExtra: true`); the existing `toggleSet` cascade persists them when the session is started. Pre-session `⟷` chain-link merge / unpair drives `applySupersetMerge` / `applySupersetSeparate` (already gated on `todayPlanState.workoutId` so DB writes are skipped pre-session). `deleteSet` cascade-collects drop children of a non-drop parent (DB cascade via `parent_set_id ON DELETE CASCADE` on persisted children; in-memory splice + `parentSetIdx` shift for survivors) — needed because deleting a parent extra used to leave an orphan drop with `parentSetIdx` pointing at a deleted slot. Two latent issues surfaced and fixed alongside: `addExtraSet` / `addDropSet` / `_addDropSetInner` now scan for the actual last populated index instead of using `length - 1` (sparse arrays after delete-extra or partial mid-prescription would land new entries on holes), and the +Drop Set button gate flipped from `length > 0` to `.some(s => s != null)` so it hides when the array is all holes. New helpers: `addPrescribedSetToPlan` (added in v3.4.8, removed in v3.4.9 per design feedback). No new helpers in data.js for the rest of the patch line — it's all guard-replacement and lazy-init through existing primitives.

End of v3.4.10: ~14,470 total lines across `js/*` + `api/*` (vs ~4,500 at end of Session B). Most growth is in `js/ui.js` (renderers + many new modal flows + per-block sort zones) and `js/data.js` (the v3 milestone helpers — superset block management, weight-mode fan-out, history-edit helpers, model resolvers).
